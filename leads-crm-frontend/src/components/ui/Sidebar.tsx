    ...
    <div className={`${isCollapsed ? 'w-20' : 'w-64'} h-screen bg-white dark:bg-neutral-900 border-r border-gray-200 dark:border-neutral-800 flex flex-col shrink-0 hidden md:flex transition-all duration-300 relative`}>
      ...
      <div className={`flex-1 py-6 ${isCollapsed ? 'px-3' : 'px-4'} space-y-1 overflow-x-hidden`}>
        ...
        <Link
          ...
          className={`flex items-center gap-3 px-3 py-2 rounded-lg font-medium transition-colors ${pathname === '/documents' ? 'bg-brand-50 text-brand-700 dark:bg-brand-900/20 dark:text-brand-400' : 'text-gray-600 dark:text-neutral-400 hover:bg-gray-50 dark:hover:bg-neutral-800'}`}
        >
          <Folder className={`w-5 h-5 shrink-0 ${isCollapsed ? 'mx-auto' : ''}`} />
